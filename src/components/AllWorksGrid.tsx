...
export default function AllWorksGrid({
  ...
  const lang = useLang();
  const w = dict.works;
  const categories = [w.categories.all, "Branding", "UI/UX", "Web"];
  const [active, setActive] = useState(categories[0]);

  const filtered = active === categories[0] ? projects : projects.filter((p) => p.category === active);
      ...
      <section className="pt-40 pb-32">
        ...
        <div className="px-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((p, i) => (
            ...
            >
              <Link href={`/${lang}/trabalhos/${p.slug.current}`} className="group block">
        ...
        {filtered.length === 0 && (
          ...
          >
            {lang === "en" ? "No projects in this category yet." : "Nenhum projeto nessa categoria ainda."}
          </p>
        )}
      </section>
