...
export async function generateMetadata({ params }: PageProps<"/[lang]/trabalhos">): Promise<Metadata> {
  const { lang } = await params;
  if (!hasLocale(lang)) return {};
  const dict = await getDictionary(lang);
  return {
    title: dict.metadata.trabalhos.title,
    description: dict.metadata.trabalhos.description,
  };
}
...
export default async function TrabalhosPage({ params }: PageProps<"/[lang]/trabalhos">) {
  const { lang } = await params;
